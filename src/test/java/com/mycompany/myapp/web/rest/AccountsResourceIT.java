...
class AccountsResourceIT {
    ...
    void getAllAccountsWithEagerRelationshipsIsEnabled() {
        when(accountsServiceMock.findAllWithEagerRelationships(any())).thenReturn(Flux.empty());

        webTestClient.get().uri(ENTITY_API_URL + "?eagerload=true").exchange().expectStatus().isOk();

        verify(accountsServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllAccountsWithEagerRelationshipsIsNotEnabled() {
        when(accountsServiceMock.findAllWithEagerRelationships(any())).thenReturn(Flux.empty());
