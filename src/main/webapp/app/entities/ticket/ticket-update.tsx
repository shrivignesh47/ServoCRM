...
import { Link, useNavigate, useParams } from 'react-router-dom';
import { Button, Row, Col, FormText } from 'reactstrap';
import { ValidatedField, ValidatedForm, ValidatedBlobField } from 'react-jhipster';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
...
const TicketUpdate = () => {
  ...
  const handleClose = () => navigate('/ticket' + location.search);
  ...
  const saveEntity = values => {
    ...
    const entity = {
      ...ticketEntity,
      ...values,
      user: users.find(user => user.id.toString() === values.user?.toString()),
    };
          ...
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField
                ...
                validate={{
                  required: { value: true, message: 'This field is required.' },
                  validate: v => !isNaN(Number(v)) || 'This field should be a number.',
                }}
              />
              <ValidatedField
                label="Subject"
              ...
              />
              <ValidatedField label="Channel" id="ticket-channel" name="channel" data-cy="channel" type="select">
                {channelValues.map(chan => (
                  <option value={chan} key={chan}>
                    {chan}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField label="Classifications" id="ticket-classifications" name="classifications" data-cy="classifications" type="select">
                {classificationValues.map(classif => (
                  <option value={classif} key={classif}>
                    {classif}
                  </option>
                ))}
              </ValidatedField>
