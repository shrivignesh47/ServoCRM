...
import { status as statusEnum } from 'app/shared/model/enumerations/status.model';
import { industry as industryEnum } from 'app/shared/model/enumerations/industry.model';
import { rating as ratingEnum } from 'app/shared/model/enumerations/rating.model';
import { social as socialEnum } from 'app/shared/model/enumerations/social.model';
import { getEntity, updateEntity, createEntity, reset } from './lead.reducer';
...
export const LeadUpdate = () => {
  ...
  const statusValues = Object.keys(statusEnum);
  const industryValues = Object.keys(industryEnum);
  const ratingValues = Object.keys(ratingEnum);
  const socialValues = Object.keys(socialEnum);
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField label="Industry" id="lead-industry" name="industry" data-cy="industry" type="select">
                {industryValues.map(industryValue => (
                  <option value={industryValue} key={industryValue}>
                    {industryValue}
                  </option>
                ))}
              </ValidatedField>
              ...
              <ValidatedField label="Rating" id="lead-rating" name="rating" data-cy="rating" type="select">
                {ratingValues.map(ratingValue => (
                  <option value={ratingValue} key={ratingValue}>
                    {ratingValue}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField label="Social Media" id="lead-social_media" name="social_media" data-cy="social_media" type="select">
                {socialValues.map(socialValue => (
                  <option value={socialValue} key={socialValue}>
                    {socialValue}
                  </option>
                ))}
              </ValidatedField>
