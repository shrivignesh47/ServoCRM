...
export const Lead = () => {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 2 }}>
        <Button variant="contained" color="info" onClick={handleSyncList} disabled={loading}>
          <FontAwesomeIcon icon="sync" spin={loading} /> Refresh list
        </Button>
        <Link to="/lead/new">
      ...
      >
        <Grid container spacing={3}>
          {leadList && leadList.length > 0 ? (
            leadList.map((lead, i) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={`entity-${i}`}>
                <Card variant="outlined" sx={{ backgroundColor: '#f5f5f5', color: '#333' }}>
                  <CardContent style={{backgroundColor:'#162c46'}}>
                    <Typography variant="h6" color="#d3e3fdb3" component="div" gutterBottom>
                      {lead.first_name} {lead.last_name}
                    </Typography>
                    <Typography variant="body2" color="#d3e3fdb3" >
                    ...
                    {lead.lead_image && (
                      <Box sx={{ mt: 2 }}>
                        <a href={URL.createObjectURL(new Blob([lead.lead_image]))} target="_blank" rel="noopener noreferrer">
                          Open Lead Image
                        </a>
                  ...
                  <CardActions>
                    <Button size="small" component={Link} to={`/lead/${lead.id}`} color="info">
                      <FontAwesomeIcon icon="eye" /> View
                    </Button>
                    <Button size="small" component={Link} to={`/lead/${lead.id}/edit`} color="primary">
